...
const Main = (function () {
  ...
  function _refreshResumeSave() {
    ...
    if (ok) {
      ...
      if (sub) {
        sub.textContent = (d.meta.side === 'srp' ? 'Brotherhood of Seth' : 'UDC') +
          ' · ' + what + ' · ' + mm + ':' + ss;
      }
    }
  }
  ...
  function _drawBriefMap(m) {
    ...
    // markers wear FACTION colors: a Brotherhood commander is RED on their own
  ...
  function _showBriefing(m) {
    ...
    $('briefClass').textContent = mySide === 'udc'
      ? 'UDC TACTICAL NET — EYES ONLY' : 'BROTHERHOOD WHISPERS — FOR THE FAITHFUL';
